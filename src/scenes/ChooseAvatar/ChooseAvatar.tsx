...
function ChooseAvatar() {
  ...
  const [source, changeSource] = useState(`data:image/svg+xml;utf8,${encodeURIComponent(createAvatar(style))}`); 

  function changeIcon(){
    let randomString = Math.random().toString(36).substring(2, 6);
    console.log("seed geradas: " + randomString);
    let src = `data:image/svg+xml;utf8,${encodeURIComponent(createAvatar(style, {seed: randomString}))}`;
    changeSource(src);
  }
